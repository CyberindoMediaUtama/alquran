...
switch($action) {
    case 'get_surahs':
        echo json_encode(getSurahList($db));
        break;
        
    case 'get_ayahs':
        $suraId = (int)($_GET['sura_id'] ?? 1);
        echo json_encode(getAyahsBySurah($db, $suraId));
        break;
        
    case 'search':
        $query = $_GET['q'] ?? '';
        echo json_encode(searchAyahs($db, $query));
        break;
        
    case 'get_ayah':
        $suraId = (int)($_GET['sura_id'] ?? 1);
        $verseId = (int)($_GET['verse_id'] ?? 1);
        echo json_encode(getAyah($db, $suraId, $verseId));
        break;
...
function getSurahList($db) {
    ...
    $surahNames = [
        ...
        105 => 'Al-Fil', 106 => 'Quraish', 107 => 'Al-Ma\'un', 108 => 'Al-Kawthar',
    ...
    foreach($surahs as $surah) {
        $result[] = [
            'id' => $surah['suraId'],
            'name' => $surahNames[$surah['suraId']] ?? 'Surah ' . $surah['suraId'],
...
function getAyahsBySurah($db, $suraId) {
    return $db->getAyahsBySurah($suraId);
}

function searchAyahs($db, $query) {
    if(strlen(trim($query)) < 2) {
        return ['error' => 'Query too short'];
    }
    return $db->searchAyahs($query);
}

function getAyah($db, $suraId, $verseId) {
    return $db->getAyah($suraId, $verseId);
}
?>